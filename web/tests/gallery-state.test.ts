...
test("gallery query accepts the canonical fields and drops unknown values when serialized", () => {
  const state = parseGalleryQuery("?typ=story&seite=02&jahr=2026&sort=secret");
  assert.deepEqual(state, { typ: "story", seite: 2, jahr: 2026 });
  assert.equal(serializeGalleryQuery(state), "?typ=story&seite=2&jahr=2026");
  assert.equal(galleryUrl("/bilder/seite/2/", state), "/bilder/seite/2/?typ=story&seite=2&jahr=2026");
});


test("invalid query values fall back without reflecting unknown parameters", () => {
  assert.deepEqual(parseGalleryQuery("?typ=unsafe&seite=0&jahr=20x6&debug=true"), { typ: "all", seite: 1, jahr: null });
  assert.equal(serializeGalleryQuery({ typ: "all", seite: 1, jahr: null }), "");
});
...
test("unknown is a distinct media category and never becomes classic", () => {
  ...
  assert.deepEqual(filterGalleryMedia(media, { typ: "unknown", seite: 1, jahr: null }).map((entry) => entry.asset_id), ["u"]);
  assert.deepEqual(filterGalleryMedia(media, { typ: "classic", seite: 1, jahr: null }).map((entry) => entry.asset_id), ["c"]);
...
test("year is parsed from the source filename and page state clamps to available results", () => {
  ...
  assert.deepEqual(normalized, { typ: "all", seite: 2, jahr: null });
  assert.equal(galleryPageCount(media, { typ: "all", seite: 1, jahr: null }), 2);
  assert.equal(galleryPageItems(media, { typ: "all", seite: 2, jahr: null }).length, 1);
});
