...
Die vier neuen Sicherheits-/Fehlersemantik-Regressionen wurden zunächst rot nachgewiesen und sind anschließend grün. Ein weiterer roter Dokumentationsvertrag entfernte danach sämtliche verbliebenen Befehle für die abgelösten Laufzeitpfade `wirtelprimpf-venv` und `~/.local/bin/wirtelprimpf_generator.py`.
