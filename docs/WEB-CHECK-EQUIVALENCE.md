...
| Web artifact and budgets | `python3 tests/test_pages_artifact.py` und `python3 scripts/validate_pages_artifact.py` sowie `python3 scripts/validate_web_budgets.py` | `web`-Job: `python3 scripts/build_web_site.py --check` | Der Build arbeitet in einem temporären Ziel, prüft vor dem atomaren Verzeichniswechsel und lässt bei Fehlern das letzte vollständige Artefakt stehen. Pflichtseiten, Canonicals, Links, geheime/lokale Pfade, Symlinks und Größenlimits blockieren vor Veröffentlichung. |
